...
if (isset($_POST['run_prediksi'])) {
    ...
    // Simpan hasil prediksi aktif ke session
    $_SESSION['last_prediction'] = $hasil_prediksi;
}
...
<div class="card mb-4">
    <div class="card-header bg-primary text-white">Uji Asumsi Klasik & Model Fit</div>
    <div class="card-body">
        <div class="row">
            <div class="col-md-6">
...
<div class="card border-primary mb-5" id="hasil-prediksi">
    <div class="card-header bg-primary text-white">Prediksi Lonjakan Penduduk</div>
    <div class="card-body">
        <form method="POST" action="#hasil-prediksi" class="row g-3 justify-content-center">
            <div class="col-md-6 text-center">
                <label class="form-label fw-bold">Masukkan Tahun yang akan diprediksi:</label>
                <div class="d-flex justify-content-center align-items-center">
                    <div class="input-group" style="max-width: 500px;">
                        <input type="number" name="p_tahun" class="form-control form-control-lg" placeholder="Contoh: 2026" value="<?= $_POST['p_tahun'] ?? '' ?>" required>
                        <button type="submit" name="run_prediksi" class="btn btn-primary px-4">RUN PREDIKSI</button>
                    </div>

                    <?php if ($hasil_prediksi): ?>
                        <a href="cetak_laporan.php" target="_blank" class="btn btn-danger px-4 ms-3 shadow-sm">
...
<script>
    ...
    new Chart(ctx, {
        type: 'line',
        data: {
            labels: <?= json_encode($js_tahun) ?>,
            datasets: [
                {
                    label: 'Total Penduduk (Y)',
                    data: <?= json_encode($js_y) ?>,
                    borderColor: '#0d6efd',
        ...
        options: {
            ...
            scales: {
                ...
                y1: { 
                    ...
                    title: { display: true, text: 'Faktor X (Jiwa)' },
